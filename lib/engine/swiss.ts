...
function basePtsFor(player: string, round: number, preds: Map<string, Map<number, { h: number; a: number }>>, results: Map<number, { h: number; a: number }>): number {
...
function predictedCount(player: string, round: number, preds: Map<string, Map<number, { h: number; a: number }>>): number {
  const pp = preds.get(player)
  if (!pp) return 0
...
export function computeSwiss(
  ...
  for (const p of profiles) {
    mp[p.player] = 0; w[p.player] = 0; d[p.player] = 0; l[p.player] = 0
    byes[p.player] = 0; played[p.player] = 0; oppsOf[p.player] = []
  }
  ...
  for (let r = 1; r <= 13; r++) {
    ...
    for (const pr of prs) {
      ...
      const out: Matchup = { a, b, aPts, bPts, res: null, ...(pr.tier != null ? { tier: pr.tier } : {}), ...(pr.slot ? { slot: pr.slot } : {}) }

      if (b && hasRemovalEffect(b, r) && !hasRemovalEffect(a, r)) {
        out.res = 'removed'; mp[a] += 3; byes[a]++
      } else if (b && hasRemovalEffect(a, r) && !hasRemovalEffect(b, r)) {
        out.res = 'removed'; mp[b] += 3; byes[b]++
      } else if (!b) {
        // SWISS-07: bye = 3 match pts, but only when the round is complete (SWISS-06)
        if (complete) { out.res = 'bye'; mp[a] += 3; byes[a]++ }
      } else if (complete) {
        ...
        if (aNo && bNo) {
          ...
          // SWISS-06: win = 3
          out.res = 'a'; mp[a] += 3; w[a]++; l[b]++
        } else if (bPts > aPts) {
          out.res = 'b'; mp[b] += 3; w[b]++; l[a]++
        } else {
          // SWISS-06: draw = 1–1
          out.res = 'draw'; mp[a]++; mp[b]++; d[a]++; d[b]++
        }

        if (out.res !== 'void') {
          played[a]++; played[b]++
          oppsOf[a].push(b); oppsOf[b].push(a)
          const key = a < b ? `${a}||${b}` : `${b}||${a}`
          const raw: MatchupResult = out.res
          h2h[key] = a < b
            ? (raw as 'a' | 'b' | 'draw')
            : (raw === 'a' ? 'b' : raw === 'b' ? 'a' : 'draw')
        }
      }
  ...
  for (const p of profiles) {
    ...
    for (const r of completed) {
      ...
      if (!inRound) continue
      if (predictedCount(p.player, r, predMap) === 0) { streak++; if (streak >= 2) hit = true }
      else streak = 0
    }
  ...
  for (let i = 0; i < rows.length - 1; i++) {
    ...
    if (tiedXY && !tiedXYZ && !prevTied) {
      ...
      const hr = h2h[key]
      if (hr && hr !== 'draw') {
        const winner = x.name < y.name
          ? (hr === 'a' ? x.name : y.name)
          : (hr === 'a' ? y.name : x.name)
        if (rows[i].name !== winner) { rows[i] = y; rows[i + 1] = x }
      }
    }
  }
