...
// Read the admin-configured claim code. Returns null when no code is set (claims open).
// Stored in imported_rows adminAuth:claimCode so no schema migration is needed.
// Fails open — if the DB read errors, treat as "no code required" to avoid locking out
// players during a DB hiccup. Set a strong code via POST /api/admin/claim-code.
export async function getClaimCode(): Promise<string | null> {
  try {
    ...
    const rows = await sql`
      ...
      LIMIT 1
    `
    const code = (rows as Array<{ payload: any }>)[0]?.payload?.code
