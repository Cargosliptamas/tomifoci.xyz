...
function useMatchView(fixture: Fixture) {
  ...
  const events = (state?.matchEvents?.[String(fixture.id)] ?? []).filter(
    (e) => CARD_EVENT_TYPES.has(e.type)
  ) as CardEvent[]
  const htScore = state?.matchScores?.[String(fixture.id)]?.ht ?? null
  return { result, live, pred, wiz, earned: byMatch, events, htScore }
}

export function LiveCard({ fixture }: { fixture: Fixture }) {
  const { result, live, pred, wiz, earned, events, htScore } = useMatchView(fixture)
...
export function FinishedCard({ fixture }: { fixture: Fixture }) {
  const { result, pred, wiz, earned, events, htScore } = useMatchView(fixture)
...
function InlineEvents({ events, htScore }: { events: CardEvent[]; htScore: { h: number; a: number } | null }) {
