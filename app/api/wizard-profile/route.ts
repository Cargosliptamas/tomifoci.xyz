...
export async function POST(request: Request) {
  let body: { player?: string; pin?: string; community?: string; active?: boolean; mirror?: boolean }
  try {
  ...
  const mirror = body.mirror !== false
  await upsertImportedRow('wizardProfiles', `${community}:${player}`, {
    player,
    community,
    active,
    mirror,
